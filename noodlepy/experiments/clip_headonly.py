...
# --- 4) Compute class weights for weighted loss ---
labels = np.array(train_dataset.labels)
class_counts = np.bincount(labels, minlength=2)
total = labels.shape[0]
# weight = total / (num_classes * count)
weights = torch.tensor([ total/(2*class_counts[0]),
                         total/(2*class_counts[1]) ],
                       dtype=torch.float)

# --- 5) Model, criterion, optimizer ---
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
model = CLIPHeadOnlyFineTuner(
    pretrained_model_name="openai/clip-vit-base-patch32",
    num_classes=2,
    dropout_p=0.7
).to(device)
...
for epoch in range(1, num_epochs + 1):
    ...
    for px, labels in train_loader:
        ...
        loss.backward()
        optimizer.step()
        train_loss += loss.item() * px.size(0)
    ...
    if val_loss < best_val_loss:
        ...
        best_model_wts = copy.deepcopy(model.state_dict())
        torch.save(best_model_wts, "best_clip_head_only.pth")
        epochs_no_improve = 0
...
# --- 7) Test evaluation with the best head-only model ---
