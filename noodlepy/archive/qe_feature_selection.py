...
def plot_all_patient_spectra(dataset):
    ...
    # load feature selection csv file
    feature_selection_file = os.path.join(os.getcwd(), "feature_selection.csv")
    ...
    feature_selection = np.loadtxt(feature_selection_file, delimiter=',', skiprows=1)  # Assuming the first row contains the selection


    # index that value is not zero
    # index = np.where(feature_selection[:,2])[0]
    features = feature_selection[:,0]
    coeff = feature_selection[:,2]
    # rescale the values between 0 and 1
    # coeff_rescale = (coeff - np.min(coeff)) / (np.max(coeff) - np.min(coeff))
    coeff_rescale = (coeff /np.max(coeff))*0.25
    ...
    feature_selection_colors = LinearSegmentedColormap.from_list("RedBlack", ["red", "black"])

    greens = plt.cm.Greens(np.linspace(0.5, 1, max(2, 1)))
    fig, ax2 = plt.subplots(figsize=(8, 6))

    # for i, feature in enumerate(features):
    #     ax2.axvline(x=feature, color=feature_selection_colors(coeff_rescale[i]), linewidth=0.5)
    # max_coeff = np.argmax(coeff_rescale)
    # ax2.axvline(x=features[max_coeff], color=feature_selection_colors(1), linewidth=0.5, label=f"Feature Selected by L1 penalized Logistic Regression")

    # plot the coefficients as one line
    ax2.plot(features, coeff_rescale, color=feature_selection_colors(1), linewidth=0.5, label="Feature Selection Coefficients")
    ...
    plt.tight_layout()
    plt.savefig(f"Differece_spectra_cancer_control_with_selected_features.png", dpi=300, transparent=True)
